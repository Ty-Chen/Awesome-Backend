...
* #### 说说innodb引擎的redo log, undo log, bin log？
  ...
  可见，redo是故障恢复及前滚（恢复事务作出的改动）。undo是事务回滚，恢复事务开始前数据的快照。
...
* #### innodb实现mvcc的基本原理是什么
  ...
  * 事务B开启，随后查询表中id=1这条数据，生成了自己的read-view。innodb内核根据**当前事务的read-view**判断当前数据的版本，拿到可以读取的数据；
...
* #### 大表怎么优化？某个表有近千万数据，CRUD比较慢，如何优化？分库分表了是怎么做的？分表分库了有什么问题？有用到中间件么？他们的原理知道么？
  ...
  使用UUID作主键是最简单的方案，但是缺点也是非常明显的。由于UUID非常的长，除占用大量存储空间外，最主要的问题是在索引上，在建立索引和基于索引进行查询时都存在性能问题。 Twitter的分布式自增ID算法Snowflake 在分布式系统中，需要生成全局UID的场合还是比较多的，twitter的snowflake解决了这种需求，实现也还是很简单的，除去配置信息，核心代码就是毫秒级时间41位 机器ID 10位 毫秒内序列12位。
...
* #### NoSQL和传统关系型数据库在写入性能上有何差异？

  一般而言，**NoSQL数据库的**写入性能要好于**关系型数据库**。

  得出这个结论，需要从各自的写入过程说起。

  首先，不论哪种数据库，对机械磁盘的访问方式分为随机IO和顺序IO。随机IO需要做**磁盘寻道**，一般来说，读写效率要比顺序IO小两到三个数量级。

  ### 传统关系型数据库

  以mysql为例，其主键是聚簇索引（数据与索引数据放在一起），在数据插入或者更新的时候，我们需要找到要插入的位置，再把数据写到特定的位置上，这个过程会产生随机IO。而且一旦发生了**页分裂**，就不可避免会做数据的移动，更加损耗写入性能。

  ### NoSQL 数据库

  很多 NoSQL 数据库都在使用的基于 **LSM树**的存储引擎。**LSM 树（Log-Structured Merge Tree）**牺牲了一定的读性能来换取写入数据的高性能，Hbase、Cassandra、LevelDB 都是用这种算法作为存储的引擎。

  数据首先会写入到一个叫做 MemTable 的内存结构中，在 MemTable 中数据是按照**写入的Key**来排序的。

  MemTable 在累积到一定规模时，它会被刷新生成一个新的文件，我们把这个文件叫做 SSTable（Sorted String Table）。当 SSTable 达到一定数量时，我们会将这些 SSTable 合并，减少文件的数量，因为**SSTable都是有序**的，所以合并的速度也很快。

  把随机IO改为顺序IO，从而提升写入性能，这就是二者差异的底层原因。

  虽然读取性能会受影响，但也不至于差多。因为存储的数据（无论是MemTable还是SSTable）都是有序的，查找效率不低，只是因为数据被拆分成多个 SSTable，所以读取的效率会低于 B+ 树索引。

---

* #### 从一条sql查询优化说起

  使用mysql数据库，innodb引擎。针对以下sql查询:
  ...
  - 能用到operate_time索引吗？
  - 能用到(type, status)联合索引吗？
  - 你还会问哪些问题，获取什么信息，以便决策建立什么样的索引 ？
  - 其它条件一样情况下，对int（4字节）字段和bigint（8字节）建立索引，哪个效果好，为什么？

  

  ##### 问题一，**不能**。

  因为**最左前缀匹配**原则，mysql会一直向右匹配直到遇到**范围查询(>、<、between、like)**就停止匹配。
  ...
  - 这张表还有哪些高频率查询，以做出综合判断。
  - 这条sql用到的几个字段区分度如何，区分度可以用公式：**count(distinct col)/count(1)**，表示字段不重复的比例。

  ##### 问题四

  **int**比**int8**好。这是由索引的数据结构**b++树**的性质决定的。
   IO越少，速度越快，而IO次数取决于b+数的高度h。假设当前数据表的数据量为N，每个磁盘块的数据项的数量是m，则有h = **㏒(m+1)N**，当数据量N一定的情况下，m越大，h越小；而m = **磁盘块的大小 / 数据项的大小**，磁盘块的大小也就是**一个数据页的大小，是固定的**，如果数据项占的空间越小，数据项的数量越多，树的高度越低。
...
* #### 并发的事务可能什么出现什么问题？
  ...
  事务的ACID特性中，隔离性(Isolation)并不是一件简单的事，实际上也不一定要实现完全隔离。那么，根据隔离程度的不同，两个并发的事务都可能出现哪些问题/现象呢？

  比如，为人所熟知的“脏读”便是其中一种。事务1读取到了事务2中未提交的数据。太可怕了，所以一般我们会要求杜绝脏读，即隔离级别至少设置为READ_COMMITTED。

---

* #### 除此之外，还会出什么问题，对应可以杜绝这个问题的是什么隔离级别？

  还有：

  - 不可重复读(Unrepeatable Read)：事务 A 读取了事务 B **已提交的更改**数据。 需要**REPEATABLE_READ**才能杜绝。
  - 幻读(Phantom Read)：事务 A 读取了事务 B **已提交的新增**数据。需要**SERIALIZABLE**级别才能杜绝。

  这两个问题出现的概率非常小，所以一般情况下使用**READ_COMMITED**隔离级别即可。

  注：
   InnDB 引擎默认的事务隔离级别是**可重复读（Repeatable Read）**，但它在该级别中用**Gap锁**把**幻读**的问题也解决了。InnDB 中事务隔离级别通过锁、**MVCC**实现。
...
- #### 为什么InnoDB 不跟 MyISAM 一样，也把数字存起来呢？

  这是因为即使是在同一个时刻的多个查询，由于多版本并发控制（MVCC）的原因，InnoDB 表“应该返回多少行”也是不确定的。按InnoDB 的事务设计，可重复读是它默认的隔离级别，在代码上就是通过多版本并发控制，也就是 MVCC 来实现的。每一行记录都要判断自己是否对这个会话可见，因此对于 count(*) 请求来说，InnoDB 只好把数据一行一行地读出依次判断，可见的行才能够用于计算“基于这个查询”的表的总行数。

  另外，InnoDB 是索引组织表，主键索引树的叶子节点是数据，而普通索引树的叶子节点是主键值。所以，普通索引树比主键索引树小很多。对于 count(*) 这样的操作，遍历哪个索引树得到的结果逻辑上都是一样的。因此，MySQL 优化器会找到最小的那棵树来遍历。在保证逻辑正确的前提下，尽量减少扫描的数据量，是数据库系统设计的通用法则之一。
...
- #### count(*)、count(主键 id)、count(字段) 和 count(1) 等不同用法的性能差别？
  ...
  - 对于 count(1) 来说，InnoDB 引擎遍历整张表，但不取值。server 层对于返回的每一行，放一个数字“1”进去，判断是不可能为空的，按行累加。因为从引擎返回 id 会涉及到解析数据行，以及拷贝字段值的操作，所以count(1)会比count(主键id)快。
...
- #### 为什么表数据删掉一半，表文件大小不变？
  ...
  首先，可能是`innodb_file_per_table`未打开。

  - 这个参数设置为 OFF 表示的是，表的数据放在系统共享表空间，也就是跟数据字典放在一起；
  - 这个参数设置为 ON 表示的是，每个 InnoDB 表数据存储在一个以 .ibd 为后缀的文件中。

  在打开的前提下，删除、增加数据均容易造成空洞：可以复用，而没有被使用的空间。要通过 alter table 命令重建表，才能达到表文件变小的目的。